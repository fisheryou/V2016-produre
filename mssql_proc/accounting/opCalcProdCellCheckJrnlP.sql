...
IF exists(SELECT 1 FROM sysobjects WHERE name = 'opCalcProdCellCheckJrnlP')
	DROP PROC opCalcProdCellCheckJrnlP
go

CREATE PROC opCalcProdCellCheckJrnlP
...
AS
SET NOCOUNT ON
  CREATE TABLE #tt_prodCellRawJrnlPHist
(
...
CREATE TABLE #tt_prodCellCheckJrnlPHist
(
  serialNO                         NUMERIC(19,0)                                      NOT NULL, -- 自增长
  createPosiDate                   VARCHAR(10)                                        NOT NULL, -- 建仓日期
  settleDate                       VARCHAR(10)                                        NOT NULL, -- 交收日期
...
--matchSettlePrice                 NUMERIC(10,4)    DEFAULT 0                         NOT NULL,
 ...
 CREATE TABLE #tt_prodCellCreatePosiDateP
(
  prodCellCode                     VARCHAR(30)                                        NOT NULL,          --产品单元代码         
...
CREATE TABLE #tt_prodCellCreatePosiDatePSum
(
  prodCellCode                     VARCHAR(30)                                        NOT NULL,          --产品单元代码
  secuAcctCode                     VARCHAR(30)                                        NOT NULL,          --证券账户
  currencyCode                     VARCHAR(3)         DEFAULT 'CNY'                   NOT NULL,          --货币代码
 ...
 --todo
  ...
  --删除产品债券核算流水
  DELETE sims2016TradeHist..prodCellCheckJrnlPHist
   ...
   --汇总质押式逆回购
   --333 逆回购 335逆回购购回
  INSERT INTO #tt_prodCellRawJrnlPHist (groupID, shareRecordDate, serialNO, settleDate,
   ...
   WHILE 1 = 1
    BEGIN
      IF @loop_fundAcctCode IS NOT NULL AND (@loop_fundAcctCode != @v_fundAcctCode OR @@FETCH_STATUS != 0)
        BEGIN
          INSERT sims2016TradeHist..prodCellCheckJrnlPHist(createPosiDate, settleDate, 
                                                            prodCode, prodCellCode, fundAcctCode, secuAcctCode, currencyCode, 
                                                            exchangeCode, secuCode, originSecuCode, secuTradeTypeCode, 
                                                            marketLevelCode, transactionNO, investPortfolioCode, buySellFlagCode, bizSubTypeCode, 
                                                            openCloseFlagCode, longShortFlagCode, hedgeFlagCode, secuBizTypeCode, 
                                                     ...
                                                     SELECT createPosiDate, settleDate, 
                                                            ...
                                                            costChgAmt, occupyCostChgAmt, rlzChgProfit, 
                                                            investCostChgAmt, investOccupyCostChgAmt, investRlzChgProfit,  
                                                            @i_operatorCode, GETDATE(), operateRemarkText FROM #tt_prodCellCheckJrnlPHist
          TRUNCATE TABLE #tt_prodCellCheckJrnlPHist         
        END 
      ...
      IF @loop_fundAcctCode IS NULL OR (@v_fundAcctCode != @loop_fundAcctCode)
        BEGIN
          ...
          TRUNCATE TABLE #tt_prodCellCreatePosiDateP
          TRUNCATE TABLE #tt_prodCellCreatePosiDatePSum
          
          INSERT INTO #tt_prodCellCreatePosiDateP(prodCellCode, secuAcctCode, currencyCode, exchangeCode, secuCode, 
          ...
          INSERT INTO #tt_prodCellCreatePosiDatePSum(prodCellCode, secuAcctCode, currencyCode, exchangeCode, secuCode, 
																											longShortFlagCode, hedgeFlagCode, marketLevelCode, transactionNO, investPortfolioCode, 
																											createPosiDate, repurchaseDate, posiQty, costChgAmt, lastOperateDate)
																							 SELECT prodCellCode, secuAcctCode, currencyCode, exchangeCode, secuCode, 
																											longShortFlagCode, hedgeFlagCode, marketLevelCode, transactionNO, investPortfolioCode,  
																											createPosiDate, repurchaseDate, SUM(posiQty), SUM(costChgAmt), MAX(lastOperateDate)
																								 FROM #tt_prodCellCreatePosiDateP
																								GROUP BY prodCellCode, secuAcctCode, currencyCode, exchangeCode, secuCode, 
      ...
      IF @v_secuBizTypeCode = '333'
        BEGIN        
					...
					INSERT INTO #tt_prodCellCreatePosiDatePSum(prodCellCode, secuAcctCode, currencyCode,
          ...
          INSERT INTO #tt_prodCellCheckJrnlPHist(serialNO, createPosiDate, settleDate,
                                                   prodCode, prodCellCode, fundAcctCode, secuAcctCode, currencyCode, 
                                                   exchangeCode, secuCode, originSecuCode, secuTradeTypeCode, 
                                                   marketLevelCode, transactionNO, investPortfolioCode, buySellFlagCode, bizSubTypeCode, 
                                                   openCloseFlagCode, longShortFlagCode, hedgeFlagCode, secuBizTypeCode, 
      ...
      ELSE IF @v_secuBizTypeCode = '335' 
        BEGIN
					...
					SELECT @costChgAmt = costChgAmt FROM #tt_prodCellCreatePosiDatePSum
...
select * from sims2016TradeHist..prodCellCheckJrnlPHist