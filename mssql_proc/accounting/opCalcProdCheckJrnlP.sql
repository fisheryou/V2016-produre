...
IF exists(SELECT 1 FROM sysobjects WHERE name = 'opCalcProdCheckJrnlP')
	DROP PROC opCalcProdCheckJrnlP
go

CREATE PROC opCalcProdCheckJrnlP
...
AS
SET NOCOUNT ON
  CREATE TABLE #tt_prodRawJrnlPHist
(
...
CREATE TABLE #tt_prodCheckJrnlPHist
(
  serialNO                         NUMERIC(19,0)                                      NOT NULL, -- 自增长
  createPosiDate                   VARCHAR(10)                                        NOT NULL, -- 建仓日期
  settleDate                       VARCHAR(10)                                        NOT NULL, -- 交收日期
...
--matchSettlePrice                 NUMERIC(10,4)    DEFAULT 0                         NOT NULL,
 ...
 CREATE TABLE #tt_prodCreatePosiDateP
(     
...
CREATE TABLE #tt_prodCreatePosiDatePSum
(
  secuAcctCode                     VARCHAR(30)                                        NOT NULL,          --证券账户
  currencyCode                     VARCHAR(3)         DEFAULT 'CNY'                   NOT NULL,          --货币代码
  exchangeCode                     VARCHAR(4)                                         NOT NULL,          --交易所代码 
 ...
 --todo
  ...
  --删除产品债券核算流水
  DELETE sims2016TradeHist..prodCheckJrnlPHist
   ...
   --汇总质押式逆回购
   --333 逆回购 335逆回购购回
  INSERT INTO #tt_prodRawJrnlPHist (groupID, shareRecordDate, serialNO, settleDate,
                              ...
                              SELECT 0 AS groupID, ' ' AS shareRecordDate, MAX(serialNO), settleDate,
                                ...
                                FROM sims2016TradeHist..prodRawJrnlPHist a        
                               WHERE settleDate >= @v_realBeginDate
                            ...
                            ORDER BY fundAcctCode, settleDate, prodCode, investPortfolioCode, secuAcctCode, currencyCode, exchangeCode, secuCode, hedgeFlagCode, marketLevelCode, secuBizTypeCode, openCloseFlagCode   
   ...
   WHILE 1 = 1
    BEGIN
      IF @loop_fundAcctCode IS NOT NULL AND (@loop_fundAcctCode != @v_fundAcctCode OR @@FETCH_STATUS != 0)
        BEGIN
          INSERT sims2016TradeHist..prodCheckJrnlPHist(createPosiDate, settleDate, 
                                                      prodCode, fundAcctCode, secuAcctCode, currencyCode, 
                                                      exchangeCode, secuCode, originSecuCode, secuTradeTypeCode, 
                                               ...
                                               SELECT createPosiDate, settleDate, 
                                                      ...
                                                      costChgAmt, occupyCostChgAmt, rlzChgProfit, 
                                                      investCostChgAmt, investOccupyCostChgAmt, investRlzChgProfit,  
                                                      @i_operatorCode, GETDATE(), operateRemarkText FROM #tt_prodCheckJrnlPHist
          TRUNCATE TABLE #tt_prodCheckJrnlPHist         
        END 
      ...
      IF @loop_fundAcctCode IS NULL OR (@v_fundAcctCode != @loop_fundAcctCode)
        BEGIN
          ...
          TRUNCATE TABLE #tt_prodCreatePosiDateP
          TRUNCATE TABLE #tt_prodCreatePosiDatePSum
          
          INSERT INTO #tt_prodCreatePosiDateP(secuAcctCode, currencyCode, exchangeCode, secuCode, 
																			 ...
																			 SELECT  secuAcctCode, currencyCode, exchangeCode, secuCode, 
																				 ...
																				 FROM sims2016TradeHist..prodCheckJrnlPHist
																				...
																				GROUP BY secuAcctCode, currencyCode, exchangeCode, secuCode, 
																							longShortFlagCode, hedgeFlagCode, marketLevelCode, settleDate
																			 HAVING SUM(matchQty) > 0
																																																																		
          INSERT INTO #tt_prodCreatePosiDatePSum( secuAcctCode, currencyCode, exchangeCode, secuCode, 
																									longShortFlagCode, hedgeFlagCode, marketLevelCode, 
																									createPosiDate, repurchaseDate, posiQty, costChgAmt, lastOperateDate)
																					 SELECT  secuAcctCode, currencyCode, exchangeCode, secuCode, 
																									longShortFlagCode, hedgeFlagCode, marketLevelCode,  
																									createPosiDate, repurchaseDate, SUM(posiQty), SUM(costChgAmt), MAX(lastOperateDate)
																						 FROM #tt_prodCreatePosiDateP
																						GROUP BY secuAcctCode, currencyCode, exchangeCode, secuCode, 
																									longShortFlagCode, hedgeFlagCode, marketLevelCode,createPosiDate, repurchaseDate
      ...
      IF @v_secuBizTypeCode = '333'
        BEGIN        
					...
					INSERT INTO #tt_prodCreatePosiDatePSum(secuAcctCode, currencyCode,
																								 exchangeCode, secuCode, longShortFlagCode,
																								 hedgeFlagCode, marketLevelCode,
																								 createPosiDate, repurchaseDate,
																								 posiQty, costChgAmt, lastOperateDate)
																					values( @v_secuAcctCode, @v_currencyCode,
																								 @v_exchangeCode, @v_secuCode, @v_longShortFlagCode,
																								 @v_hedgeFlagCode, @v_marketLevelCode,
																								 @v_settleDate, dbo.fn_get_ghrq(@v_settleDate, @v_exchangeCode, @v_secuCode),
																								 @v_matchQty, @v_matchSettleAmt, ' ')
         						                     
          INSERT INTO #tt_prodCheckJrnlPHist(serialNO, createPosiDate, settleDate,
                                             prodCode, prodCellCode, fundAcctCode, secuAcctCode, currencyCode, 
                                             exchangeCode, secuCode, originSecuCode, secuTradeTypeCode, 
                                             marketLevelCode, transactionNO, investPortfolioCode, buySellFlagCode, bizSubTypeCode, 
                                             openCloseFlagCode, longShortFlagCode, hedgeFlagCode, secuBizTypeCode, 
      ...
      ELSE IF @v_secuBizTypeCode = '335' 
        BEGIN
					...
					SELECT @costChgAmt = costChgAmt FROM #tt_prodCreatePosiDatePSum
...
select * from sims2016TradeHist..prodCheckJrnlPHist
